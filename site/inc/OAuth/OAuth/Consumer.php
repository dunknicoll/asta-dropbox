...
require_once JPATH_SITE . '/components/com_dropbox/inc/OAuth/OAuth/Consumer/Request.php';
require_once JPATH_SITE . '/components/com_dropbox/inc/OAuth/OAuth/Consumer/Exception/InvalidResponse.php';
...
class HTTP_OAuth_Consumer extends HTTP_OAuth
{
    ...
    public function getRequestToken($url, $callback = 'oob',
        ...
        if (empty($data['oauth_token']) || empty($data['oauth_token_secret'])) {
            throw new HTTP_OAuth_Consumer_Exception_InvalidResponse(
                'Failed getting token and token secret from response (request token)', $response
            );
        }
    ...
    public function getAccessToken($url, $verifier = '',
        ...
        if (empty($data['oauth_token']) || empty($data['oauth_token_secret'])) {
            throw new HTTP_OAuth_Consumer_Exception_InvalidResponse(
                'Failed getting token and token secret from response (access token)', $response
            );
        }
